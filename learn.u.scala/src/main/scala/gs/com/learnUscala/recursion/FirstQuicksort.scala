...
class FirstQuicksort {
  ...
  def quicksort(xs: List[Int]): List[Int] = {
    ...
    def sort(pivot: Int, xs: List[Int])(ltEq: ListBuffer[Int], gt: ListBuffer[Int]):Unit = {
      for (x <- xs) {
        splitLtEqGt(pivot, x)(ltEq, gt)
      }
    }
    def quicksortAcc(xs: List[Int]):List[Int] = xs match {
      case Nil => xs
      case head :: tail =>
        val ltEq = new ListBuffer[Int]()
        val gt = new ListBuffer[Int]()
        sort(head, tail)(ltEq, gt)
        val small = quicksortAcc(ltEq toList)
        val big = quicksortAcc(gt toList)
